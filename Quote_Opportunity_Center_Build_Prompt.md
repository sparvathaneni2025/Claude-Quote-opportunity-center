...
- **Lost quotes are never formally closed.** Every $5,000+ quote checked with a `lostCode` set (DEMANDCHG, PRICING, NO BID, PROJ-LOST, etc. — over 100 checked) still shows `status = 'Quote Issued'` or `'Quote WIP'`. There is no reliable way to compute a Lost bucket from current BC data. This needs a **process fix** (reps/BC admin actually closing quotes), not a data fix. A separate one-page instructional doc for the team covers this. **Re-verified at the 2026-09-15 refresh, and sharper:** of the 112 active $5K+ quotes pulled, 28 have `lostCode` set and **zero** have `reasonCode` set — including all 28 with a `lostCode`. Reps are tagging a risk code without ever pairing it with a reason, on top of never closing the quote out. See the Lost Quotes tab (Section 7, item 11).
- **`status = 'Converted to Order'` means an order was created, not that it shipped or invoiced.** Cross-checking against `salesOrders.shipped` and `invoices.quoteNo` showed 2 of 3 confirmed Won quotes hadn't shipped yet. Track funnel stage (Order Created → Shipped/Invoiced), not a flat Won/Lost binary. Still true at the 2026-09-15 refresh (3 current-system Won quotes, one still unshipped/uninvoiced).
- **BC was recently upgraded**, changing quote numbering from an old "SQ" prefix to the current "SQIE" prefix (same business, same accounts, ~April 2026 cutover). A large, ongoing volume of real revenue (**$1,059,243.50** confirmed since April 2026 as of the original build, **$2,004,436.31** as of the 2026-09-15 refresh, still posting) is legacy pre-upgrade business finishing its lifecycle against old "SQ" quote numbers — this is why the current system's own Won total ($30,609.74 → $36,709.25 at the refresh) looks tiny in isolation. Keep these two populations visually distinct (Archive's "System" column: Current vs. Legacy) rather than merging them into one number.
- **The 2,000-row query cap is the single most dangerous failure mode in this build.** It silently drops data rather than erroring, and it looks identical to "there's genuinely nothing there." Always filter tightly (by account list, by status, by amount threshold) rather than pulling broadly and filtering client-side. **Re-verified and worse over time:** by the 2026-09-15 refresh, a single `$orderby=documentDate desc` pass over legacy "SQ" invoices only reached back to 2026-07-27 before hitting the cap (it reached ~April 13, 2026 at the original build) — accumulating volume shrinks the achievable lookback window on every refresh. Pulling the same April-2026-onward range now takes 3 chained desc passes (each filtered `documentDate lt` the prior pass's earliest date), not 1.
...
QUOTES = [{
  ...
  lostCode, reasonCode, // header-level BC fields, added to support the Lost Quotes view (Section 7, item 11)
...
11. **Lost Quotes** — quote-level (not line-level) table of every `QUOTES` entry where `lostCode` and/or `reasonCode` is populated, sourced from the same per-rep pull as My Quote Queue (no separate BC query — these quotes already show up in the active pull per the finding above, since BC never closes them). Columns: quote number, customer, owner, quoteTotal, lostCode, reasonCode, status (still shows Quote Issued/WIP — expected, not a bug). Rows where exactly one of `lostCode`/`reasonCode` is set are flagged and sorted first, then by `quoteTotal` descending.

**Interactive filters (added 2026-09-15 refresh):** on My Quote Queue, Rep View, Manager View, and Lost Quotes, every KPI/summary tile and every inline flag badge is a clickable quick-filter — click filters the table/list to just the matching rows, click the same one again clears it. Only one quick-filter active at a time per view (v1); a "Showing: <label> (<count>)" indicator with a clear/✕ appears while filtered. This is separate from and stacks with the existing Quote Type/Classification/Customer dropdown filters on Queue/Rep View. Implementation: `computeFlags()` tags every flag with a stable `key`; `tableFilters[containerId].quick` (Queue/Rep View) and `managerQuickFilter`/`lostQuickFilter` (Manager/Lost Quotes) hold the active key.
...
- **Item-level classification** (the precise version, not the customer-level fallback) is carried forward refresh-to-refresh for lines that already had it (174 of 415 lines as of the 2026-09-15 refresh), but every new or changed line falls back to customer-level. The SalesModel `ItemKey`/`ItemNumber` join bug (Section 3b) needs a real fix — likely filtering item lists in smaller batches — before this can be computed fresh for every line on every refresh.
- **Legacy Won pull reaches back to ~April 2026**, but doing so takes more `$orderby desc` passes every refresh as volume grows (1 pass at build time, 3 passes by 2026-09-15 — see Section 5). Going back further than April 2026 needs even more passes against the 2,000-row cap.
- **Real Lost tracking** is a process problem, not a tool problem — see the separate instructional doc and the leadership proposal doc for Craig/Joanna/Jonathan. The Lost Quotes tab (Section 7, item 11) makes the tagged-but-never-closed quotes visible, but doesn't fix the underlying BC habit.
