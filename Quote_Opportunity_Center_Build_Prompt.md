...
9. **Lost Quotes view needs no separate pull.** Once step 2's per-rep `salesQuotes` results are in hand, filter client-side for rows where `lostCode` is non-empty (regardless of status) — that's the entire Lost Quotes dataset. `reasonCode` is already in the same `$select`, so the flag (lostCode set / reasonCode blank, or vice versa) is computed from data already on hand.

---

## 5. Real findings baked into this build (don't re-litigate, but do keep re-verifying as things change)

- **Lost quotes are never formally closed.** Every $5,000+ quote checked with a `lostCode` set (DEMANDCHG, PRICING, NO BID, PROJ-LOST, etc. — over 100 checked) still shows `status = 'Quote Issued'` or `'Quote WIP'`. There is no reliable way to compute a Lost bucket from current BC data. This needs a **process fix** (reps/BC admin actually closing quotes), not a data fix. A separate one-page instructional doc for the team covers this. *Re-verified 2026-09-09: still holds — 27 of 27 lost-coded $5K+ quotes in this pull are still open (Quote Issued / Quote WIP / Pending Approval).*
- **NEW (2026-09-09): `reasonCode` is essentially never populated alongside `lostCode`.** 0 of 27 lost-coded quotes in this pull had both fields filled in — every one has `lostCode` set and `reasonCode` blank. This is now surfaced row-by-row in the Lost Quotes view (Section 7, item 11) rather than just noted in Data Quality.
- **`status = 'Converted to Order'` means an order was created, not that it shipped or invoiced.** Cross-checking against `salesOrders.shipped` and `invoices.quoteNo` showed 2 of 3 confirmed Won quotes hadn't shipped yet. Track funnel stage (Order Created → Shipped/Invoiced), not a flat Won/Lost binary. *Re-verified 2026-09-09: still holds — 1 of 3 confirmed Won quotes this pull (Penn Cable Assembly, SQIE0013412) has an order but hasn't shipped/invoiced yet.*
- **BC was recently upgraded**, changing quote numbering from an old "SQ" prefix to the current "SQIE" prefix (same business, same accounts, ~April 2026 cutover). A large, ongoing volume of real revenue (**$1,059,243.50** confirmed since April 2026, still posting as of early August) is legacy pre-upgrade business finishing its lifecycle against old "SQ" quote numbers — this is why the current system's own Won total ($30,609.74) looks tiny in isolation. Keep these two populations visually distinct (Archive's "System" column: Current vs. Legacy) rather than merging them into one number. *Re-verified 2026-09-09: still holds — $244,989.36 across 39 legacy invoices posted 2026-07-20 through 2026-09-04 this pull (narrower window/smaller total than the prior pass, consistent with the backlog gradually winding down), vs. $36,709.25 confirmed Won under the current SQIE system. Also newly confirmed: BC has SQ-prefixed invoice history for these accounts reaching back to 2023, not just ~April 2026 — this refresh didn't pull dollar amounts for that fuller 2023+ population (out of scope for this pass), but a future refresh could.*
...
- **NEW (2026-09-09): two outlier quote amounts are worth a standing manual sanity check, not an automatic assumption of error** — `SQIE0029470` (Anduril Industries, Adam Purcell) at $2,519,335 and `SQIE0015681` (Lockheed Martin, Jerik Wong) at $954,912 together make up more than half of this pull's $6.45M active total. If either turns out to be a data-entry error in a future refresh, the active-value KPI will look very different once corrected.
...
LOST_QUOTES = [{ quoteNo, customer, owner, quoteTotal, lostCode, reasonCode, status, flagged }]
// derived client-side from the same per-rep salesQuotes pull as QUOTES — rows where lostCode is non-empty.
// flagged = true when lostCode is set but reasonCode is blank (or vice versa). Pre-sorted: flagged first, then quoteTotal desc.
```
...
1. **My Quote Queue** — all active $5K+ lines, team-wide. Filterable by Quote Type (Bid/Buy/Not set), Classification, Customer. Columns include Stock (flagged red at zero). Flags: Overdue, Due Today, Missing Follow-Up (date/code/salesperson), Expiring ≤5 days, Owner≠BC Salesperson, Missing Bid/Buy. KPI tiles and inline flag badges are clickable filters (one active filter at a time — see note below).
2. **Rep View** — same table, scoped to one rep via a dropdown, own KPIs, same clickable-filter behavior.
...
6. **Manager View** — team rollup (value, overdue, missing follow-up, zero-activity accounts per rep) + exception-based coaching queue (every flagged line, ranked by severity then value, not just top-N by dollar). Rollup-table Overdue/Missing Follow-Up counts and coaching-queue flag badges are clickable filters on the coaching queue.
...
11. **Lost Quotes** — every $5K+ quote (same per-rep pull as My Quote Queue) where `lostCode` is populated. Columns: quote number, customer, owner, quote total, lost code, reason code, status (status will still show Quote Issued/WIP — known BC limitation, expected, not a bug). Flags rows where lostCode is set but reasonCode is blank (or vice versa); sorted flagged-first, then quote total descending. "Missing Reason Code" KPI tile and the mismatch badge are clickable filters, same one-at-a-time pattern as the other views.

**Interactive filters (v1):** on My Quote Queue, Rep View, Manager View, and Lost Quotes, KPI/summary count tiles and inline flag badges are clickable — click filters the table/list to matching rows, click again toggles off. Only one active filter at a time per view; a second click on a different filter replaces the first rather than combining. A "Showing: `<label>` (N)" indicator with a clear/× button appears whenever a filter is active. Implemented via a shared `activeFilters`/`toggleFilter`/`matchesFilter`/`bindFilterClicks` helper (keyed by panel/container id) rather than duplicated per view.
